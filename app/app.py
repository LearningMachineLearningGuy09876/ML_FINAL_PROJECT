...
def load_compressed_model(path):
    """Load a compressed .pkl.gz model file."""
    with gzip.open(path, "rb") as file:
        return pickle.load(file)


def save_compressed_model(model, path):
    """Save the model as a compressed .pkl.gz file."""
...
def cached_model(train, movies, genres):
    ...
    # Preferred option for GitHub/deployment: smaller compressed model file.
    if os.path.exists(compressed_model_path):
        try:
            return load_compressed_model(compressed_model_path)
        except Exception as error:
            st.warning(f"Compressed model could not be loaded, retraining model. Error: {error}")

    # Backward-compatible fallback: regular .pkl file if you still have one locally.
    if os.path.exists(MODEL_PATH):
        try:
            return HybridRecommender.load(MODEL_PATH)
        except Exception as error:
            st.warning(f"Model file could not be loaded, retraining model. Error: {error}")

    # Final fallback: train during startup and save a compressed model for next time.
