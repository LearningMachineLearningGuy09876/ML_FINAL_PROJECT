    ...
    st.markdown(
        ...
        <style>
            ...
            .hero-logo {{
                max-width: 780px;
                width: 100%;
                filter: drop-shadow(0px 8px 22px rgba(0,0,0,0.45));
            }}
        ...
        <div class="hero-wrapper">
            <div class="hero-top">
                <img class="hero-logo" src="data:image/jpg;base64,{logo_base64}">
            </div>
...
def search_tmdb_person(person_name):
    """Find an actor, actress, or director on TMDB by name."""
...
@st.cache_data(show_spinner=False)
def get_tmdb_person_movie_credits(person_id):
    """Return acting and directing movie credits for one TMDB person."""
    ...
    try:
        ...
        acted_movies = data.get("cast", [])
        directed_movies = [
            movie for movie in data.get("crew", [])
            if movie.get("job", "").lower() == "director"
        ]

        return {"acted": acted_movies, "directed": directed_movies}
    except Exception:
        return {"acted": [], "directed": []}


def tmdb_movies_to_dataframe(tmdb_movies, person_name="", credit_type="acted"):
    """Convert TMDB credit results into the same shape the card UI expects."""
    rows = []
    ...
    for movie in tmdb_movies:
        ...
        rows.append({
            ...
            "genres": f"TMDB {credit_type.title()} Credit",
    ...
    if not df.empty:
        df = df.drop_duplicates(subset=["title"]).sort_values(
...
def lookup_tmdb_person_movies(person_name, role="any", limit=50):
    """Lookup person credits for the chatbot when local data has no cast/crew columns."""
    person = search_tmdb_person(person_name)
    if not person:
        return pd.DataFrame()

    credits = get_tmdb_person_movie_credits(person["id"])

    if role == "director":
        movies_list = credits.get("directed", [])
        credit_type = "directed"
    elif role in ["actor", "cast"]:
        movies_list = credits.get("acted", [])
        credit_type = "acted"
    else:
        # Combine both, but keep acting results first because most searches are actor searches.
        movies_list = credits.get("acted", []) + credits.get("directed", [])
        credit_type = "acted/directed"

    return tmdb_movies_to_dataframe(
        movies_list[:limit],
...
def main():
    ...
    chatbot = MovieChatbot(
        model=model,
        movies_df=movies,
        person_lookup_func=lookup_tmdb_person_movies,
    )
    profile_store = UserProfileStore(USER_RATINGS_PATH)

    with st.sidebar:
        sidebar_logo_path = os.path.join(BASE_DIR, "assets", "flikpik_logo.jpg")
        ...
        st.write(f"TMDB Cast/Crew: **{'On' if TMDB_API_KEY else 'Off'}**")
    ...
    with tabs[2]:
        ...
        st.caption("Search movies by actor, actress, or director using TMDB credits.")

        if not TMDB_API_KEY:
            st.warning("TMDB_API_KEY is missing. Add your TMDB key to enable actor/director search.")

        person_query = st.text_input(
            "Search actor, actress, or director",
            placeholder="Example: Tom Hanks, Zendaya, Christopher Nolan",
            key="person_search_input",
        )

        credit_filter = st.radio(
            "Search by",
            ["Acting Roles", "Directed Movies", "Both"],
            horizontal=True,
            key="person_credit_filter",
        )
        ...
        if person_query:
            ...
            if not person:
                st.warning("No person found. Try another name or check spelling.")
            else:
                st.subheader(f"Results for {person.get('name', person_query)}")
                credits = get_tmdb_person_movie_credits(person["id"])

                if credit_filter == "Acting Roles":
                    actor_df = tmdb_movies_to_dataframe(
                        credits.get("acted", []),
                        person_name=person.get("name", person_query),
                        credit_type="acted",
                    )
                elif credit_filter == "Directed Movies":
                    actor_df = tmdb_movies_to_dataframe(
                        credits.get("directed", []),
                        person_name=person.get("name", person_query),
                        credit_type="directed",
                    )
                else:
                    both_movies = credits.get("acted", []) + credits.get("directed", [])
                    actor_df = tmdb_movies_to_dataframe(
                        both_movies,
                        person_name=person.get("name", person_query),
                        credit_type="acted/director",
                    )

                if actor_df.empty:
                    st.info("No movies found for that cast/crew search.")
                else:
                    st.caption(f"Found {len(actor_df)} movie credits. Showing the highest-popularity matches first.")
                    show_movie_cards(actor_df, "weighted_score", max_items=20) if display == "Cards" else show_movie_table(actor_df, limit=30)
    ...
    with tabs[9]:
        ...
        st.caption("Ask for movies by mood, genre, actor, director, decade, year, or similarity. Try: 'movies with Denzel Washington', 'directed by Christopher Nolan', or 'like Inception after 2000'.")
        ...
        if st.button("Ask Assistant", type="primary"):
            ...
            if parsed:
                parsed_cols = st.columns(5)
                parsed_cols[0].metric("Mood", mood)
                parsed_cols[1].metric("Genres", ", ".join(parsed.get("genres", [])) or "Any")
                parsed_cols[2].metric("Person", parsed.get("person") or "Any")
                parsed_cols[3].metric("Role", parsed.get("person_role") or "Any")
                year_label = parsed.get("year") or parsed.get("decade") or parsed.get("year_min") or "Any"
                parsed_cols[4].metric("Year Filter", year_label)
