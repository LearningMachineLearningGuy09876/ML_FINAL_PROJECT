...
def show_movie_cards(df, score_col=None, max_items=24):
    ...
    for row in df.to_dict("records"):
        ...
        # Use a real TMDB poster when available.
        poster = row.get("poster_url") or fetch_tmdb_poster(title)

        # If TMDB is missing or no real poster is found, fall back to a clean placeholder.
        # This keeps the section visible instead of disappearing.
        if not poster:
            poster = placeholder_poster(title)

        # Only validate real TMDB posters. Placeholder posters should always display.
...
def main():
    ...
    if page == "🏠 Home":
        ...
        if global_search:
            ...
            # Try to attach real TMDB posters, but do NOT delete the whole section
            # if posters are missing. show_movie_cards() will use placeholders as fallback.
            if TMDB_API_KEY:
            ...
            if not TMDB_API_KEY:
                st.info("TMDB_API_KEY is missing, so placeholder posters are shown until your TMDB key is added.")

            st.caption("Scroll inside this section to browse more trending movies.")

            with st.container(height=720, border=True):
                show_movie_cards(trending, "hype", max_items=48) if display == "Cards" else show_movie_table(trending)
